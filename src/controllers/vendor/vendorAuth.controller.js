...
const signup = async(req, res) => {
    try {
        const { brandName, brandEmail, mobileNumber, password, confirmPassword } = req.body;
        //validation
        const errorMessage = formValidator.validateSignUp(brandName, brandEmail, mobileNumber, password, confirmPassword)
        if(errorMessage){
        ...
        return res.status(200).json({ message: "OTP sent to your email" , redirect: "/vendor/verify-otp" });
